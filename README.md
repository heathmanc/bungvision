...
- Display polish: fixed a long-standing `MetricCard` styling bug where the `QFrame` border/background rule cascaded onto the card's child labels and boxed every line of text (a `QLabel` is a `QFrame`); the rule is now scoped to the card via object name, so the operator-screen metric cards (PASS RATE / REJECTS / INFERENCE) and the dashboard cards render as clean stat tiles. Dashboard tables now size to their content (no clipped/overlapping rows) and use a dark, high-contrast alternate-row color instead of the pale theme default.
